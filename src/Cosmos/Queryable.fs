...
type IQueryable<'T> with
    ...
    member inline query.MapAsyncEnumerable<'T, 'Result>
        (mapping : IterationState<'T> -> 'T -> 'Result, [<Optional; EnumeratorCancellation>] cancellationToken : CancellationToken)
        =
        query.ToFeedIterator().MapAsyncEnumerable<'T, 'Result> (mapping, cancellationToken)

    member inline query.MapFoldAsyncEnumerable<'T, 'State, 'Result>
        (
